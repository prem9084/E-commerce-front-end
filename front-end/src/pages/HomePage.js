...
const HomePage = () => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `https://g-mart.onrender.com/api/v1/products/products-list/${page}`
      );
      setLoading(false);
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get(
        "https://g-mart.onrender.com/api/v1/products/products-count"
      );
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
  ...
  const filterProducts = async () => {
    try {
      const { data } = await axios.post(
        "https://g-mart.onrender.com/api/v1/products/products-filter",
        {
          checked,
          radio,
        }
